...
// Verifica se o código da compra foi passado pela URL
if (isset($_GET['codcompra'])) {
    $codcompra = $_GET['codcompra'];

    // Verifica se o formulário de confirmação foi submetido
    if (isset($_POST['confirm'])) {
        try {
            // Executa a exclusão da compra
        ...
        // Busca os detalhes da compra para confirmar a exclusão
        try {
    ...
    header('Location: consulta-compra.php');
    exit;
}
?>
...
<div class="card-exc">
    <div class="header">
        ...
        <div class="content">
            <?php if ($message): ?>
                <span class="title"><?php echo ($messageType == 'success') ? 'Compra Excluída' : 'Erro na Exclusão'; ?></span>
                <p class="message"><?php echo $message; ?></p>
                <?php if ($messageType == 'success'): ?>
                    <p class="message">Você será redirecionado em 2 segundos...</p>
                <?php else: ?>
                    <a href="consulta-compra.php" class="cancel">Voltar</a>
                <?php endif; ?>
            <?php elseif (isset($tb_compras)): ?>
                <span class="title">Excluir Compra</span>
                <p class="message">Tem certeza que deseja excluir a compra <strong><?php echo htmlspecialchars($tb_compras['codcompra']); ?></strong>? Esta ação não poderá ser desfeita.</p>
            </div>
            <div class="actions">
                <form method="post" action="">
                    <button type="submit" name="confirm" class="desactivate">Confirmar Exclusão</button>
                    <a href="consulta-compra.php" class="cancel">Cancelar</a>
                </form>
            <?php else: ?>
